...
def index(request):
    user = User.objects.create_user(username='zhiliao',email='zhiliao@qq.com',password ='1111112')
...
![image-20200630130701324](images/image-20200630130701324.png)
...
```python
#如果模型是一个代理模型，可以增加属性，但是不能增加字段
#那么久不能再这个模型中添加新的Field
```
